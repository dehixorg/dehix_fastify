...
export class FreelancerDAO extends BaseDAO {
  ...
  async getFreelancerByReferralCode(referralCode: string) {
    try {
      return await this.model.findOne({ "referral.referralCode": referralCode });
    } catch (error) {
  ...
  async addReferralBonus(referrer_id: string, freelancer_id: string) {
    try {
      ...
      const referrerUpdate = await this.updateFreelancerData(
        referrer_id,
        {
      ...
      const freelancerUpdate = await this.updateFreelancerData(
        freelancer_id,
        {
      ...
      return { referrerUpdate, freelancerUpdate };
    } catch (error) {
