import { Controller, GET, Inject, POST, PUT, Service } from "fastify-decorators";
import { AuthController } from "../common/auth.controller";
import { CREATE_INTERVIEW_END_POINT, GET_ALL_INTERVIEW, INTERVIEW, UPDATE_INTERVIEW_END_POINT } from "../constants/interview.constant";
...
import { getInterviewSchema } from "src/schema/v1/interview/get";
...
async createInterview(request:FastifyRequest<{Params:GetInterviewPathParams,Body:InterviewBody}>,reply:FastifyReply){
try {
   this.logger.info("controllers->interview.controller->createInterview");
   const data=await this.InterviewService.createInterview(request.params.interviewee_id,request.body)
...
async updateInterviewById(request:FastifyRequest<{Params:GetInterviewPathParams,Body:updateInterviewBody}>,reply:FastifyReply){
try {
   this.logger.info("controllers->interview.controller->updateInterview")
   const data= await this.InterviewService.updateInterview(request.params.interview_id,request.body)
   reply.status(STATUS_CODES.SUCCESS).send({data})
} catch (error:any) {
   reply.status(STATUS_CODES.SERVER_ERROR).send({message:"Internal server error"})
}
}
