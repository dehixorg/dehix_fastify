...
import {
  CONSULTANT_END_POINT,
  CREATE_CONSULTANT_END_POINT,
  DELETE_CONSULTANT_END_POINT,
  GET_ALL_CONSULTANT_END_POINT,
  GET_CONSULTANT_BY_ID_END_POINT,
  UPDATE_CONSULTANT_END_POINT,
} from "../constants/consultant.constant";
import { createConsultantSchema } from "../schema/v1/consultant/create";
import {
  getAllConsultantSchema,
  getConsultantSchema,
} from "../schema/v1/consultant/get";
...
import {
  PutConsultantBody,
  PutconsultantPathParams,
} from "../types/v1/consultant/put";
...
export default class ConsultantController extends AuthController {
  ...
  async createConsultant(
    request: FastifyRequest<{ Body: PostConsultantBody }>,
    reply: FastifyReply,
  ) {
  ...
  async updateConsultant(
    request: FastifyRequest<{
      Params: PutconsultantPathParams;
    ...
    try {
      ...
      const data = await this.ConsultantService.updateConsultantById(
      ...
      if (
        ...
        error.message.includes(
          "Consultant with provided ID could not be found.",
        )
      ) {
  ...
  async getConsultantById(
    ...
    try {
      const data = await this.ConsultantService.getConsultantById(
        request.params.consultant_id,
      );
      reply.status(STATUS_CODES.SUCCESS).send({ data });
    } catch (error: any) {
  ...
  async deleteConsultantById(
    request: FastifyRequest<{ Params: DeleteconsultantPathParams }>,
    reply: FastifyReply,
  ) {
    this.logger.info(
      "Controllers->consultant.controller-> deleteConsultantById",
    );
    try {
      await this.ConsultantService.deleteConsultantById(
        request.params.consultant_id,
      );
      reply
