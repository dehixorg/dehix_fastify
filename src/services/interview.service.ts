...
export class InterviewService extends BaseService {
  ...
  async createInterview(interviewee_id: string, interview_data: any) {
    ...
    const userExist =
      await this.freelancerDao.findFreelancerById(interviewee_id);
    if (!userExist) {
    ...
    const findFreelancer =
      await this.freelancerDao.getInterviewer(interviewee_id);
  ...
  async updateInterview(interview_id: string, update: any) {
    ...
    const data = await this.interviewDao.updateInterviewById(
      interview_id,
      update,
    );
    return data;
  ...
  async getInterviewWithInterviewerId(interviewer_id: string) {
    this.logger.info(
      "service->interview.service->getInterviewWithInterviewerId",
    );
    const data =
