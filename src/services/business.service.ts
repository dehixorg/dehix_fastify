...
export class BusinessService extends BaseService {
  ...
  async updateBiddingDate(project_id: string, maxBiddingDate: Date, startBiddingDate: Date) {
    ...
    if (startBiddingDate.getTime() < startOfToday.getTime()) {
      throw new Error("startBiddingDate cannot be in the past. It must be today or a future date.");
    }
    
    const data = await this.ProjectDAO.updateBiddingDate(project_id, maxBiddingDate, startBiddingDate);
    return data;
  }
}
