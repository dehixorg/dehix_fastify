...
const pricingTypeEnum = z.enum(['VENUE_RENTAL','MIN_SPEND_PER_PERSON','MIN_SPEND_PRICINGS','ACCOMMODATION'])
const pricingDurationEnum = z.enum(['PER_HOUR','PER_SESSION','PER_MORNING','PER_AFTERNOON','PER_EVENING','PER_DAY'])
const pricingDaysEnum = z.enum(['MONDAY','TUESDAY','WEDNESDAY','THURSDAY','FRIDAY','SATURDAY','SUNDAY'])
...
const additionalDetailsSchema = z.object({
  ...
  const locationSchema = z.object({
    ...
    accessibility_features: z.array(z.string()).nonempty('At least one accessibility feature is required'),
...
const offersAndPackageSchema = z.array(z.object({
...
const cateringAndDrinksSchema = z.object({
...
const spaceRuleSchema = z.object({
...
const facilitiesAndEquipmentsSchema = z.object({
...
const photosAndVideosSchema = z.object({
  images: z.array(z.string().nonempty()).nonempty('At least one image is required'),
  videos: z.array(z.string().nonempty()).nonempty('At least one video is required'),
  video_upload_type: z.string(videoUplodeTypeEnum).nonempty('Video Upload type'),
}).strict();

const uploadDocumentsSchema = z.object({
  floor_plans : z.array(z.string().nonempty()).nonempty('At least one image is required'),
  trade_licenses : z.array(z.string().nonempty()).nonempty('At least one image is required'),
  sample_menus : z.array(z.string().nonempty()).nonempty('At least one image is required'),
})
