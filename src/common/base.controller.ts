...
import { FastifyInstanceToken, Inject, ErrorHandler, Hook } from 'fastify-decorators';
...
export abstract class BaseController {
  ...
  async onReplySend(request, reply, payload, done) {
    ...
    reply.header('Access-Control-Allow-Methods', 'OPTIONS, GET, PUT, PATCH, POST, DELETE');
  ...
  handleTokenNotFound(error: BadTokenError, request: FastifyRequest, reply: FastifyReply) {
    reply.status(403).send({ message: 'Not Authorized', code: error.statusCode });
  }
  ...
  handleBadRequestError(error: UnAuthorisedError, request: FastifyRequest, reply: FastifyReply) {
    reply.status(400).send({ message: error.message, code: error.statusCode });
  }

  @ErrorHandler(ConflictError)
  handleConflictError(error: UnAuthorisedError, request: FastifyRequest, reply: FastifyReply) {
    reply.status(409).send({ message: error.message, code: error.statusCode });
  }

  @ErrorHandler(CreationError)
  handleCreationError(error: CreationError, request: FastifyRequest, reply: FastifyReply) {
    reply.status(500).send({ message: error.message, code: error.statusCode });
  }

  @ErrorHandler(ServerError)
  handleServerError(error: ServerError, request: FastifyRequest, reply: FastifyReply) {
    reply.status(500).send({ message: error.message, code: error.statusCode });
  }
  ...
  handleDataValidationError(error: DataValidationError, request: FastifyRequest, reply: FastifyReply) {
    this.fastifyInstance.log.error(`DataValidationError: error ${error.message}`);
    reply.status(400).send({ message: error.message, code: error.statusCode, data: error?.data });
  }

  @ErrorHandler(ERROR_CODES.FST_ERR_VALIDATION)
  handleSchemaValidationError(error: DataValidationError, request: FastifyRequest, reply: FastifyReply) {
  ...
  @ErrorHandler()
  generalErrorHandler(error: any, request: FastifyRequest, reply: FastifyReply) {
